<script setup>
import { ref } from 'vue'
import RecipeService from '@/services/RecipeService'
import AddAttachment from '@/components/attachments/AddAttachment.vue'
import { useRoute } from 'vue-router'

const route = useRoute()
const slug = route.params.slug
const recipe = ref()

const loadFromServer = () => {
  RecipeService.getRecipe(slug)
    .then(data => {
      recipe.value = data
    })
    .catch(error => {
      // ? do anything here?s
    })
}

loadFromServer()
</script>

<template>
  <div>
    <section class="text-white bg-orange" v-if="recipe">
      <div class="py-12 width-small-wrapper">
        <h4 class="text-sm text-white uppercase">Rezept</h4>
        <h2 class="mb-4 text-3xl font-bold text-white">{{ recipe.title }}</h2>
        <p>{{ recipe.description }}</p>
        <p class="mt-4 text-sm">
          Erstellt am {{ $filters.formatedDateTime(recipe.created_at) }}
        </p>
        <p class="mt-4 text-sm">
          Zuletzt bearbeitet {{ $filters.formatedDateTime(recipe.updated_at) }}
        </p>
      </div>
    </section>
    <section v-if="recipe" class="my-8 width-wrapper">
      <h2>Zutaten</h2>
      <div class="grid grid-cols-3 gap-4 rounded-md aspect-square">
        <div
          class="p-4 text-white bg-green"
          v-for="ingredient in recipe.attached_urls.concat(
            recipe.attached_files,
          )"
        >
          <h4>{{ ingredient.title }}</h4>
          <p>{{ ingredient.description }}</p>
        </div>
      </div>
      <h2 class="mt-16">Weitere Zutaten hinzufügen</h2>
      <add-attachment :recipe="recipe" @changed="loadFromServer" />
    </section>
  </div>
</template>
